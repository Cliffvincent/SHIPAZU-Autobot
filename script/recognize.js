...
module.exports.config = {
  ...
  aliases: ["recog", "regco"],
  hasPrefix: false,
  usage: "{p}{n} reply to short video",
...
module.exports.run = async function ({ api, event }) {
  try {
    ...
    const music = messageReply.attachments[0].url;

    const res = await axios.get(`https://yt-video-production.up.railway.app/recognize?fileUrl=${encodeURIComponent(music)}`
    );
    ...
    api.sendMessage("Not found Recognize that music" + error.message, event.threadID, event.messageID
    );
  } finally {
